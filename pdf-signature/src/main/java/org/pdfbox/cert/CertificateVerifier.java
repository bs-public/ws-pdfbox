...
public final class CertificateVerifier {
  ...
  /**
   * Attempts to build a certification chain for given certificate and to verify it. Relies on a set
   * of root CA certificates and intermediate certificates that will be used for building the
   * certification chain. The verification process assumes that all self-signed certificates in the
   * set are trusted root CA certificates and all other certificates in the set are intermediate
   * certificates.
   *
   ...
   *        All the rest are considered to be intermediate CA certificates.
   ...
   * @throws CertificateVerificationException - if the certification is not successful (e.g.
   *         certification path cannot be built or some certificate in the chain is expired or CRL
   *         checks are failed)
   */
  public static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert,
    ...
    try {
      ...
      PKIXCertPathBuilderResult verifiedCertChain =
          verifyCertificate(cert, trustAnchors, intermediateCerts, signDate);
      ...
      throw cvex;
    } catch (IOException | URISyntaxException | GeneralSecurityException
        | RevokedCertificateException | OCSPException ex) {
      throw new CertificateVerificationException(
          "Error verifying the certificate: " + cert.getSubjectX500Principal(), ex);
    }
  }

  private static void checkRevocations(X509Certificate cert, Set<X509Certificate> additionalCerts,
      Date signDate) throws IOException, CertificateVerificationException, OCSPException,
  ...
  /**
   * Attempts to build a certification chain for given certificate and to verify it. Relies on a set
   * of root CA certificates (trust anchors) and a set of intermediate certificates (to be used as
   * part of the chain).
   *
   * @param cert - certificate for validation
   ...
   *         path cannot be built or some certificate in the chain is expired)
   */
  private static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert,
      Set<TrustAnchor> trustAnchors, Set<X509Certificate> intermediateCerts, Date signDate)
    ...
    // (when using -Djava.security.debug=certpath: "critical policy qualifiers present in
    // certificate")
  ...
  private static String extractOCSPURL(X509Certificate cert) throws IOException {
    ...
    if (authorityExtensionValue != null) {
      ...
      ASN1Sequence asn1Seq =
          (ASN1Sequence) JcaX509ExtensionUtils.parseExtensionValue(authorityExtensionValue);
      Enumeration<?> objects = asn1Seq.getObjects();
      while (objects.hasMoreElements()) {
        // AccessDescription
        ASN1Sequence obj = (ASN1Sequence) objects.nextElement();
