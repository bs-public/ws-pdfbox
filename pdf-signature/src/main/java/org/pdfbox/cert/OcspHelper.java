...
public class OcspHelper {
   ...
   * @param additionalCerts Set of trusted root CA certificates that will be used as "trust anchors"
   *        and intermediate CA certificates that will be used as part of the certification chain.
   *        All self-signed certificates are considered to be trusted root CA certificates. All the
   *        rest are considered to be intermediate CA certificates.
   * @param ocspUrl where to fetch for OCSP
   */
  public OcspHelper(X509Certificate checkCertificate, Date signDate,
      X509Certificate issuerCertificate, Set<X509Certificate> additionalCerts, String ocspUrl) {
   ...
   * This method should be used instead of {@code basicResponse.getCerts()[0]}
   *
  ...
  private OCSPResp performRequest(String urlString)
    ...
    try {
      ...
      if (responseCode == HttpURLConnection.HTTP_MOVED_TEMP
        ...
        if (urlString.startsWith("http://") && location.startsWith("https://")
  ...
  private OCSPReq generateOCSPRequest() throws OCSPException, IOException {
    ...
    Extension nonceExtension =
        new Extension(OCSPObjectIdentifiers.id_pkix_ocsp_nonce, false, encodedNonce);

    OCSPReqBuilder builder = new OCSPReqBuilder();
    builder
        .setRequestExtensions(new Extensions(new Extension[] {responseExtension, nonceExtension}));
