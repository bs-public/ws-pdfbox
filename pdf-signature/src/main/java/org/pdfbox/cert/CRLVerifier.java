...
public final class CRLVerifier {
    ...
    try {
      ...
      for (String crlDistributionPointsURL : crlDistributionPointsURLs) {
        ...
        if (crlIssuerCert == null) {
          throw new CertificateVerificationException("Certificate for "
              + crl.getIssuerX500Principal() + "not found in certificate chain, so the CRL at "
  ...
  public static void checkRevocation(X509CRL crl, X509Certificate cert, Date signDate,
    ...
    if (revokedCRLEntry != null && revokedCRLEntry.getRevocationDate().compareTo(signDate) <= 0) {
      throw new RevokedCertificateException("The certificate was revoked by CRL "
      ...
      LOG.info("The certificate was revoked after signing by CRL " + crlDistributionPointsURL
          + " on " + revokedCRLEntry.getRevocationDate());
    } else {
   ...
   */
  private static X509CRL downloadCRL(String crlURL) throws IOException, CertificateException,
      CRLException, CertificateVerificationException, NamingException, URISyntaxException {
  ...
  public static List<String> getCrlDistributionPoints(X509Certificate cert) throws IOException {
    ...
    if (!(derObjCrlDP instanceof ASN1OctetString)) {
      LOG.warn("CRL distribution points for certificate subject "
          + cert.getSubjectX500Principal().getName() + " should be an octet string, but is "
          + derObjCrlDP);
      return new ArrayList<>();
    }
