...
import org.pdfbox.cert.CertificateVerifier;
...
public class CertInformationCollector {
  ...
  /**
   * Processes an embedded signed timestamp, that has been placed into a signature. The certificates
   * and its chain(s) will be processed the same way as the signature itself.
   *
  ...
  /**
   * Processes a signer store and goes through the signers certificate-chain. Adds the found data to
   * the certInfo. Handles only the first signer, although multiple would be possible, but is not
   * yet practicable.
   *
   ...
   */
  private SignerInformation processSignerStore(CMSSignedData signedData,
  ...
  private void traverseChain(X509Certificate certificate, CertSignatureInformation certInfo,
    ...
    if (certInfo.issuerCertificate == null) {
      throw new IOException("No Issuer Certificate found for Cert: '"
  ...
  /**
   * Get alternative certificate chain, from the Authority Information (a url). If the chain is not
   * included in the signature, this is the main chain. Otherwise there might be a second chain.
   * Exceptions which happen on this chain will be logged and ignored, because the cert might not be
   * available at the time or other reasons.
   *
  ...
  /**
   * Gets a list of X509Certificate out of an array of X509CertificateHolder. The certificates will
   * be added to certificateSet.
   *
   ...
   * @throws CertificateProccessingException
   */
  CertSignatureInformation getCertInfo(X509Certificate certificate)
      throws CertificateProccessingException {
    try {
